...
def finalize_trial(
    bundle: Path, directory: Path, record: dict[str, Any], failpoint: str = ""
) -> None:
...
def trial(
    ...
    *,
    ...
    try:
        ...
        finalize_trial(bundle, directory, record, failpoint)
    return record
    ...
    *,
    ...
    try:
        ...
        for trial_id in manifest["inventory"]:
            ...
            write(output / "manifest.json", manifest)
            try:
                trial(output, subject, scratch, run_id, trial_id, failpoint, record=record)
            except Exception as exc:
