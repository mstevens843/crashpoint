...
def source_inventory(subject: Path) -> dict[str, str]:
    head = subprocess.check_output(["git", "-C", str(subject), "rev-parse", "HEAD"], text=True)
    need(head.strip() == PIN, "subject_pin")
    paths = subprocess.check_output(["git", "-C", str(subject), "ls-files", "-z"]).split(b"\0")
    inventory = {}
    for raw in paths:
        if not raw:
            continue
        ...
        committed = subprocess.check_output(["git", "-C", str(subject), "show", f"{PIN}:{name}"])
...
def freeze(subject: Path, plan_path: Path) -> None:
    need(not plan_path.exists(), "plan_already_exists")
    subject_hashes = source_inventory(subject)
    own = {name: digest((ROOT / name).read_bytes()) for name in OWN_SOURCES}
    predictions = {
...
def trial(
    bundle: Path, subject: Path, scratch: Path, run_id: str, trial_id: str, failpoint: str = ""
) -> dict[str, Any]:
    ...
    try:
        ...
        if case in {"pre_crash", "post_crash", "unknown_error"}:
            outcome = "SUCCEEDED" if before["effects"] == 1 else "FAILED"
            call(
                "reconcile",
                "reality.action.reconcile",
                {
                    ...
                    "outcome": outcome,
                    "evidence_note": (
                        f"fixture receiver sha256={before['sha256']}; effects={before['effects']}"
                    ),
                },
                url,
            )
        ...
        record["artifacts"] = {
            p.name: digest(p.read_bytes()) for p in sorted(directory.iterdir()) if p.is_file()
        }
        if record["evidence_valid"]:
            from .reality_layer_verify import derive_trial

            try:
                record["findings"] = derive_trial(bundle, record)
            except Exception as exc:
                record["evidence_valid"] = False
                record["errors"].append({"type": type(exc).__name__, "message": str(exc)})
        try:
            ...
            record["evidence_valid"] = False
            record["errors"].append({"type": type(exc).__name__, "message": str(exc)})
            # If this also fails, propagate visibly; never claim retention succeeded.
...
def capture(
    ...
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "inventory": [only] if only else INVENTORY,
        "cases": CASES,
        "plan_sha256": None,
        "source_hashes": {},
    ...
    try:
        ...
        plan = parse(plan_raw)
        need(plan["subject_sources"] == source_inventory(subject), "subject_source_changed")
        ...
        need(
            subprocess.check_output(["node", "--version"], text=True).strip() == "v22.22.1",
            "node_pin",
        )
        ...
        for trial_id in manifest["inventory"]:
            record = trial(output, subject, scratch, run_id, trial_id, failpoint)
