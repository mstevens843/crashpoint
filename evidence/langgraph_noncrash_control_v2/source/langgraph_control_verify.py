...
def verify_bundle(bundle_root: Path) -> list[str]:
    ...
    record, problems = _read_json_file(bundle_root / "receipt.json", "receipt.json")
    if record is None:
        return problems
    ...
    contract_body, contract_problems = _read_json_file(
        bundle_root / str(contract["path"]), "contract.json"
    )
    ...
    manifest_body, manifest_problems = _read_json_file(
        bundle_root / str(manifest["path"]), "manifest.json"
    )
    ...
    if report_path_declared is None:
        ...
        report_body, report_problems = _read_json_file(
            bundle_root / str(report_path_declared), "observer report"
        )
