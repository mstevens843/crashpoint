...
def verify_bundle(bundle_root: Path) -> dict[str, Any]:
    ...
    for trial in trials:
        ...
        if "admission.sqlite" in confined and isinstance(action_id, str):
            ...
            completions = read_completion_rows(confined["admission.sqlite"], action_id)
            # Two distinct admitted actions with identical arguments must never be merged: no
            # completion row here may belong to a DIFFERENT action_id than this trial's own.
            if any(c.get("action_id") != action_id for c in completions):
                problems.append(
                    Problem(trial_id, "admission.sqlite completions include a row for a "
                                       "different action_id than this trial's own")
                )
