...
def validate_receipt(rec: dict[str, Any]) -> list[str]:
    ...
    if sweep_return_value is not None:
        if not isinstance(sweep_return_value, int) or isinstance(sweep_return_value, bool):
            problems.append(f"sweep_return_value must be an int or null, got {sweep_return_value!r}")
    ...
    if measured_age is not None:
        if not isinstance(measured_age, (int, float)) or isinstance(measured_age, bool):
            problems.append(f"measured_age_at_decision must be numeric or null, got {measured_age!r}")
        elif measured_age < 0:
            problems.append(
    ...
    # Case-shape consistency: a PASSING trial's worker_a_killed must match what the case
    # structurally requires - settled_control never kills Worker A, every other case must, or the
    # rest of the passing protocol (barrier observed, TTL crossed, sweep invoked) could not have
    # happened the way the receipt otherwise claims.
    if rec["passed"] is True:
        case = rec["case"]
        killed = rec["worker_a_killed"]
        if case == "settled_control" and killed is not False:
            problems.append("settled_control never kills Worker A, but worker_a_killed is not False")
        if case != "settled_control" and killed is not True:
            problems.append(f"case {case!r} requires Worker A to be killed, but worker_a_killed is not True")
