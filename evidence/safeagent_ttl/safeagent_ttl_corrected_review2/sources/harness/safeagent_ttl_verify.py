...
def _expected_post_sweep_status(case: str, release: str) -> str | None:
    """The row's expected status in claim_post_sweep.sqlite, or None meaning sweep should have
    deleted it entirely (0.1.23's stale-PENDING delete, on the two cases that actually cross
    expiry and sweep). settled_control's row is COMMITTED throughout - sweep only ever touches
    PENDING rows, so a settled claim is untouched, not merely 'still PENDING'."""
    if case == "settled_control":
        return "COMMITTED"
    if case == "pending_before_ttl":
        return "PENDING"  # nothing was stale yet; sweep is a no-op on both releases
    if case in {"pending_expired_swept", "pre_effect_expired_swept"}:
        return None if release == "0.1.23" else "PENDING"
    return "PENDING"  # pending_expired_no_sweep never snapshots post-sweep; unreachable in practice
...
def verify_bundle(bundle_root: Path) -> dict[str, Any]:
    ...
    # --- require the complete harness/ledger/canonical source inventory independently of
    # whatever manifest.embedded_source_sha256 itself lists. The loop above only ever validates
    # entries the manifest's own dict happens to contain, so a bundle that quietly dropped a
    # harness file's key (and its sources/ copy) from that dict - e.g. the verifier's own source,
    # or ledger/core.py - would pass with nothing checking that the required inventory is
    # complete. REQUIRED_EMBEDDED_SOURCE_FILES is imported directly from safeagent_ttl_receipt so
    # this required set cannot be edited to match a short manifest. sqlite_store.py entries are
    # filtered to releases actually present in release_info, matching the runner's own
    # required_for_this_run scoping for a partial-release run.
    required_for_this_bundle = tuple(
        rel_path
        for rel_path in REQUIRED_EMBEDDED_SOURCE_FILES
        if not rel_path.startswith("safeagent_exec_guard/") or rel_path.split("/")[1] in release_info
    )
    for rel_path in required_for_this_bundle:
    ...
    for trial in trials:
        ...
        if "ledger.jsonl" in confined:
            ...
            if obs.status == "OK":
                # Lineage, not just count: this experiment's own construction (worker-a always
                # attempts before worker-b's retry is even launched - see safeagent_ttl.py's
                # trial body) means worker-a's attempt_id can never legitimately follow
                # worker-b-retry's in the raw ledger. classify_effect_count only compares
                # digests/counts and would not notice two correct attempts recorded in the
                # wrong order, so this is checked independently, directly against the raw,
                # re-parsed attempt sequence.
                raw_attempt_ids = obs.attempt_ids_by_intent.get(action_id, [])
                attempt_suffixes = [
                    raw_id.rsplit(":", 1)[1] if isinstance(raw_id, str) and ":" in raw_id
                    else raw_id
                    for raw_id in raw_attempt_ids
                ]
                known_attempts = {"worker-a", "worker-b-retry"}
                unrecognized = [s for s in attempt_suffixes if s not in known_attempts]
                if unrecognized:
                    problems.append(
                        Problem(
                            trial_id,
                            f"raw ledger attempt IDs for {action_id!r} include unrecognized "
                            f"attempt(s) {unrecognized!r} (full sequence={raw_attempt_ids!r})",
                        )
                    )
                elif (
                    "worker-a" in attempt_suffixes
                    and "worker-b-retry" in attempt_suffixes
                    and attempt_suffixes.index("worker-a")
                    > attempt_suffixes.index("worker-b-retry")
                ):
                    problems.append(
                        Problem(
                            trial_id,
                            f"raw ledger attempt order for {action_id!r} is reversed: "
                            f"worker-b-retry precedes worker-a (sequence={raw_attempt_ids!r}); "
                            "worker-a's original attempt must precede any worker-b retry",
                        )
                    )
        ...
        if "claim_post_sweep.sqlite" in confined and case in _REQUIRED_EXTRA_FILES_BY_CASE:
            ...
            expected_status = _expected_post_sweep_status(cast(str, case), cast(str, rel))
            status_mismatch = expected_status is not None and (
                row is None or row.get("status") != expected_status
            )
            if status_mismatch:
                problems.append(
                    Problem(trial_id, f"claim_post_sweep.sqlite expected a {expected_status} "
                                       f"row for case={case!r} release={rel!r} but found {row!r}")
                )
            if expected_status is None and row is not None:
                problems.append(
        ...
        if "worker_b.stdout" in confined:
            ...
            # retry_role() (safeagent_ttl_runtime.py) always emits retry_claim before deciding
            # admitted, for every case including settled_control - so a passing trial can never
            # legitimately be missing this event. Checking its content only when present (the
            # old behavior) let a bundle that had stripped the event entirely sail through.
            if claim_evt is None:
                if trial.get("passed") is True:
                    problems.append(
                        Problem(trial_id, "passed=true but retained worker_b.stdout shows no "
                                           "'retry_claim' event")
                    )
            elif claim_evt.get("admitted") != receipt_admitted:
                problems.append(
        ...
        # --- bind this trial's own provenance claims to the manifest-level release_info for its
        # release, so a per-trial version/hash cannot be fabricated independently of the run's
        # actual, once-recorded release identification ---
        rel_info = release_info.get(cast(str, rel), {}) if isinstance(release_info, dict) else {}
        if isinstance(rel_info, dict):
            for trial_field, info_field in (
                ("safeagent_version", "version"),
                ("safeagent_dist_sha256", "wheel_sha256"),
                ("safeagent_sqlite_store_sha256", "sqlite_store_sha256"),
            ):
                if trial.get(trial_field) != rel_info.get(info_field):
                    problems.append(
                        Problem(trial_id, f"{trial_field}={trial.get(trial_field)!r} does not "
                                           f"match release_info[{rel!r}].{info_field}="
                                           f"{rel_info.get(info_field)!r}")
                    )
    ...
    # The manifest's own self-reported trial_count/trials_per_cell/expected_trial_count are
    # separate fields from the actual manifest.trials list - a declaration can disagree with the
    # inventory it is supposedly describing even when the inventory itself is intact.
