...
needs neither, and it answers before you ship. If the identity is not recomputable, the dedup has
nothing stable to match on and the boundary cannot hold - so NOT_RECOMPUTABLE predicts DIVERGED at
the b1 barrier, and that implication is what this module checks by running both.
...
from ..adapters.base import _PAYLOAD
...
def verdict(observed_keys: list[str | None], intent_id: str = _INTENT) -> str:
    ...
    expected = recompute_from_durable_inputs(intent_id)
...
def probe(runtime_id: str, ledger: LedgerHandle) -> dict[str, object]:
...
def main() -> int:
    ...
    print(
        ...
        f"before any crash, and NOT_RECOMPUTABLE predicted DIVERGED at b1 - while the "
        f"forbidden-field guard accepted the payload that broke the dedup."
    )
