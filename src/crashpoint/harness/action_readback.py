...
_NEVER_READ_FALLBACK: dict[str, Any] = {
...
def _run_observer(ledger_path: Path, archive_out: Path, action_id: str) -> dict[str, Any]:
    ...
    failed read). A failed OBSERVATION (the harness could not even launch/complete the observer
    subprocess) must never abort the whole trial the way a failed admission/dispatch does - the
    harmless effect the observer was trying to look at may well have already happened; only the
    ability to look at it failed, which is exactly what NEVER_READ/UNAVAILABLE exists to record
    honestly rather than losing the rest of the trial's genuinely captured evidence."""
    try:
        proc = subprocess.run(
        ...
        # No process ever ran, so there is genuinely no raw stdout/stderr to retain here.
        return {**_NEVER_READ_FALLBACK, "_observer_exit_status": None, "_observer_stdout": "",
                "_observer_stderr": "",
                "_observer_launch_error": f"observer subprocess could not be launched: {exc!r}"}
    events = _parse_events(proc.stdout)
    result = _find_event(events, "readback_result")
    if result is None:
        return {
            **_NEVER_READ_FALLBACK,
            "_observer_exit_status": proc.returncode,
            "_observer_stdout": proc.stdout, "_observer_stderr": proc.stderr,
            "_observer_launch_error": f"no readback_result event: stdout={proc.stdout!r}",
        }
    result["_observer_exit_status"] = proc.returncode
    result["_observer_stdout"] = proc.stdout
    result["_observer_stderr"] = proc.stderr
...
def run_trial(
    ...
    try:
        ...
        # The observer's raw stdout/stderr are retained as their own files (the same pattern as
        # worker_a.stdout/worker_b.stdout) so the offline verifier can independently re-parse
        # what the observer process itself actually printed - genuinely separate evidence from
        # observer_report.json, which this same harness process wrote from the same data and so
        # cannot serve as independent confirmation of it.
        _retain_worker_stdio(
            trial_root, "observer", obs.pop("_observer_stdout", ""),
            obs.pop("_observer_stderr", ""),
        )
    ...
    with (
        ...
        for case in cases:
            for index in range(k_per_cell):
                ...
                # independently cross-checkable, crash-resilient records. A failure HERE (the
                # trial itself already succeeded; only recording it failed) must not crash the
                # whole batch or silently drop the trial from manifest.trials - it is recorded as
                # its own execution_failures entry, distinct from the trial's own result.
                if receipts and receipts[-1].get("trial_id") == trial_id:
                    try:
                        ...
                        execution_failures.append(
                            {"trial_id": trial_id, "reason": f"journal append failed: {exc!r}"}
                        )
                ...
                try:
                    ...
                    # Storage may no longer be usable at all - nothing further can be durably
                    # recorded for this trial. Re-raise so the caller sees this as fatal rather
                    # than silently continuing with a manifest.json the disk refused to accept.
                    raise RuntimeError(
                        f"manifest finalization failed after trial {trial_id!r}: {exc!r}"
                    ) from exc
