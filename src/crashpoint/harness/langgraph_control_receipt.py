...
fails after the ledger already holds real bytes.

v3. A second review reproduced three further gaps: an unvalidated ``cast(dict, json.loads(...))``
in the harness crashed on a malformed observer report (``[]``/``null``/``{}``/invalid JSON) with no
receipt ever written; the offline verifier's path joins had no confinement boundary, so an absolute
or ``..``-escaping declared path (or a symlink inside the bundle pointing outside it) could read
files outside the bundle and still verify clean; and a nonzero observer exit status alongside an
otherwise well-formed ``OBSERVED`` report still counted as agreement. v3 adds: the harness validates
observer-report shape/types before trusting anything in it (``langgraph_control.
_validate_observer_report_shape``); a new required, nullable ``observer.error`` field so a
non-OBSERVED status always carries its own honest diagnostic in the receipt itself, not just in an
ephemeral in-memory fallback; ``compute_agreement`` additionally requires ``observer_exit_status ==
0``; and the offline verifier resolves every artifact path through one checked boundary
(``langgraph_control_verify._resolve_in_bundle``) that rejects absolute paths and anything that
resolves outside the bundle root. The new required field is why this is a schema version bump, not
just a logic fix: v2 bundles recorded before this change are missing ``observer.error`` and are
correctly rejected by schema mismatch, exactly like v1 bundles are - not silently reinterpreted
under v3 rules. See the early schema check in ``validate_receipt`` and
``handoff/langgraph-noncrash-control/CODEX-CLOSEOUT-RESPONSE.md``'s second section.
"""
...
SCHEMA = "crashpoint.langgraph_control.receipt.v3"
