...
def validate_receipt(rec: dict[str, Any]) -> list[str]:
    ...
    # external_outcome/observation_availability/client_claim must themselves be the pure
    # functions of the RAW fields they are derived from (effect_count, digests_match_admission,
    # observer_raw_ledger_state, observer_exit_status, worker_a/b_local_receipt, worker_a_killed) -
    # not independently-settable strings a mutation could set to agree with each other, or with
    # the frozen prediction, while leaving the raw evidence fields fabricated. Without this, a
    # trial whose ledger-based cross-check never runs at all (unavailable_readback has no
    # ledger.jsonl retained) has nothing constraining effect_count/digests_match_admission to be
    # consistent with observed_result at all.
    recomputed_external_outcome = derive_external_outcome(
        rec.get("effect_count"),
        rec.get("digests_match_admission"),
    )
    observer_exit_status = rec.get("observer_exit_status")
    observer_exit_ok = observer_exit_status == 0 if observer_exit_status is not None else None
    recomputed_availability = derive_observation_availability(
        rec.get("observer_raw_ledger_state"),  # type: ignore[arg-type]
        observer_exit_ok,
    )
    final_local_receipt = (
        rec.get("worker_b_local_receipt")
        if rec.get("worker_b_used")
        else rec.get("worker_a_local_receipt")
    )
    worker_killed_before_receipt = bool(
        (rec.get("worker_b_used") and rec.get("worker_b_exit_status") != 0)
        or (not rec.get("worker_b_used") and rec.get("worker_a_killed"))
    )
    recomputed_claim = derive_client_claim(
        final_local_receipt=final_local_receipt,
        worker_killed_before_receipt=worker_killed_before_receipt,
    )
    if isinstance(observed, dict):
        for key, recomputed in (
            ("external_outcome", recomputed_external_outcome),
            ("observation_availability", recomputed_availability),
            ("client_claim", recomputed_claim),
        ):
            if observed.get(key) != recomputed:
                problems.append(
                    f"observed_result[{key!r}]={observed.get(key)!r} is not what the raw "
                    f"evidence fields imply ({recomputed!r})"
                )
    ...
    # The receipt carries these values twice: flattened at the top level AND nested inside
    # observed_result. The offline verifier only ever reads the nested copy (grep-confirmed), so
    # without this check the top-level copy is a completely unconstrained duplicate a mutation
    # could set to anything - including a value that contradicts the one actually checked.
    if isinstance(observed, dict):
        for key in (
            "external_outcome", "observation_availability", "client_claim", "protocol_valid",
        ):
            if rec.get(key) != observed.get(key):
                problems.append(
                    f"top-level rec[{key!r}]={rec.get(key)!r} disagrees with "
                    f"observed_result[{key!r}]={observed.get(key)!r}"
                )
        if rec.get("externally_verified") is not observed.get("externally_verified"):
            problems.append(
                f"top-level rec['externally_verified']={rec.get('externally_verified')!r} "
                f"disagrees with observed_result['externally_verified']="
                f"{observed.get('externally_verified')!r}"
            )
    ...
    # Unconditional (not gated on passed/case): worker_b_used=False must mean no worker_b fields
    # were fabricated regardless of what case or pass/fail state the trial is in - this is the
    # receipt's own internal coherence, not a case-appropriateness rule.
    if rec["worker_b_used"] is False:
        for field in (
            "worker_b_pid", "worker_b_attempt_id", "worker_b_exit_status",
            "worker_b_local_receipt",
        ):
            if rec.get(field) is not None:
                problems.append(
                    f"worker_b_used is False, but {field}={rec.get(field)!r} is not null"
                )
